...
function saveRoast(roast: Roast) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ roast, ts: Date.now() })); } catch { /* noop */ }
}
...
export default function MemeMe() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      const until = saved.ts + COOLDOWN_MS;
      if (until > Date.now()) {
        setRoast(saved.roast);
  ...
  const handleTrigger = () => {
    if (onCooldown && roast) { setVisible(true); return; }
    generateFresh();
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 22 }}>
      ...
      {!visible && (
        <button onClick={handleTrigger} style={{ ...ctaBtn, animation: onCooldown ? "none" : "cta-pulse 2.4s ease-in-out infinite" }}>
          <Sparkles size={16} />
          {onCooldown && cooldownUntil
            ? t("roastCard.viewMine")
            : t("roastCard.cta")}
        </button>
      )}
          ...
          >
            <div style={{
              ...
              <button
                ...
                aria-label="Close"
                style={{
                  position: "absolute", top: 10, left: 10, zIndex: 2, width: 24, height: 24,
                  borderRadius: "50%", border: "1px solid rgba(255,255,255,0.25)", background: "rgba(0,0,0,0.4)",
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              ...
              {onCooldown && cooldownUntil && (
                <p style={{ fontFamily: mono, fontSize: 11, color: "rgba(255,255,255,0.4)", margin: 0 }}>
                  {t("roastCard.cooldownHint", { time: formatCountdown(cooldownUntil - now) })}
                </p>
              )}
            </div>
          </motion.div>
